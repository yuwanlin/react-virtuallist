...
function VirtualList(props: VirtualListProps) {
  ...
  const updateVisibleList = (scrollTop: number) => {
    ...
    const startOffset = visibleItmesPositionRef.current[startIndexRef.current].top - visibleItmesPositionRef.current[0].top;
    console.log("TCL: updateVisibleList -> startOffset", startOffset)

    listOffsetRef.current = {
      startOffset: lastScrollItemPosition.current.top - lastBeforeBufferItemPosition.current.bottom,
      endOffset,
    }
